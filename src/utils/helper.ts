...
function convertBox(initBox: number[], imageDim: any, initFormat: "CCWH" | "XYWH" | "XYXY" = 'XYXY', initNormalized = false, finalFormat: "CCWH" | "XYWH" | "XYXY" = 'CCWH', finalNormalized = false, isDebug = false): [number, number, number, number] {
...
async function base64ToArray(base64Image: string) {
  try {
...
async function resize(
  ...
  const { width = 1, height = 1 } = await sharp(bufferedImage).metadata();
